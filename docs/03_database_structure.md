...
- **`archive` (MAM - Message Archive Management)**: Crucial for multi-device sync. If Alice sends a message from her phone, her desktop app pulls from this SQL table to display the historical message. Ejabberd writes to this table asynchronously to prevent database latency from slowing down real-time chat delivery. The new Rediff archive-search API reads from this table for the current user's tenant/vhost scope to power global message search.
