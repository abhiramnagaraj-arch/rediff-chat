...
- **`user_auth`**: Separating auth from the `users` table ensures that queries pulling user profiles never accidentally select or leak password hashes into memory.
